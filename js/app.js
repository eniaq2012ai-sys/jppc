...
function listen(){
  ...
  onSnapshot(query(collection(db,"gallery"),orderBy("order","asc")),s=>{data.gallery=s.docs.map(d=>({id:d.id,...d.data()}));renderGallery()});
}
listen();renderSports();
...
async function compressImage(file,max=1500,quality=.72,targetKB=650){
  const bmp=await createImageBitmap(file);let scale=Math.min(1,max/Math.max(bmp.width,bmp.height));
  ...
  for(let attempt=0;attempt<5;attempt++){
    ...
    if(blob.size/1024<=targetKB || attempt===4){
    ...
    w=Math.round(w*.82);h=Math.round(h*.82);quality=Math.max(.5,quality-.06)
  }
}
$$("[data-upload-asset]").forEach(b=>b.onclick=async()=>{
  if(!guard())return;const kind=b.dataset.uploadAsset;const input=kind==="logo"?$("#logoFile"):$("#heroFile");const f=input.files?.[0];if(!f)return alert("Pilih file dulu.");
  const dataUrl=await compressImage(f,kind==="logo"?1000:1800,kind==="logo"?.82:.72,650);
  await setDoc(doc(db,"assets",kind),{data:dataUrl,updatedAt:serverTimestamp()});alert("Uploaded")
});
...
function openEditor(type,id){
  ...
  if(type==="gallery")h=`<label>Label<input id="gLabel" value="${esc(found.label||"JPPC")}"></label><label>Photo<input id="gPhoto" type="file" accept="image/*"></label>${found.data?`<img src="${found.data}" style="width:100%;max-height:210px;object-fit:cover;border-radius:12px">`:""}`;
  $("#editFields").innerHTML=h;modal("editModal")
}
$("#saveItemBtn").onclick=async()=>{
  if(!guard()||!editing)return;const {type,id}=editing;let obj={updatedAt:serverTimestamp()};
  ...
  if(type==="member"){obj={...obj,name:$("#mName").value,role:$("#mRole").value,avatar:$("#mAvatar").value};const f=$("#mPhoto").files?.[0];if(f)obj.photo=await compressImage(f,1000,.72,600)}
  if(type==="ranking")obj={...obj,name:$("#rName").value,played:+$("#rPlayed").value,w:+$("#rW").value,l:+$("#rL").value};
  if(type==="gallery"){obj={...obj,label:$("#gLabel").value};const f=$("#gPhoto").files?.[0];if(f)obj.data=await compressImage(f,1600,.72,650);else if(id==="new")return alert("Pilih foto.")}
  if(id==="new"){obj.order=Date.now();await addDoc(collection(db,list),obj)}else await updateDoc(doc(db,list,id),obj);
